    ...
    <motion.section 
      ...
      <motion.p
        ...
        className="[text-shadow:_2px_2px_0px_#ed3833,_-2px_-2px_0px_#ed3833] 
                  ...
                  absolute top-1/2 -translate-y-1/2 w-full text-[8.8vw] font-bold text-center z-0 tracking-tight lg:tracking-normal whitespace-nowrap"
      >
        AGAINST LOW TRENDS
      </motion.p>
      ...
      <motion.div
        ...
        <div className="w-[90%] md:w-4/5 lg:w-1/3">
          <motion.img
            ...
            className="w-full min-h-[60vh] aspect-[4/5] md:aspect-[5/4] object-cover cursor-pointer"
        ...
        >
          ...
          >
            ...
            {selectedSection === 'works' && (
              <div className="flex flex-col items-center">
                {data.social_links && Object.entries(data.artist_works).map(([platform, link]) => link && (
                  <p key={platform} className=" flex font-bold text-2xl md:text-3xl lg:text-4xl hover:text-white transform transition-colors duration-200">
            ...
            {selectedSection === 'social media' && (
              <div className="flex flex-col items-center">
                ...
                {data.artist_presspack && (
                  <a href={data.artist_presspack}
                  target="blank"
                  rel="noopener noreferrer"
                  className="flex font-bold text-2xl md:text-3xl lg:text-4xl hover:text-white transform transition-colors duration-200">
                    presspack
                  </a>
                )}
              </div>
          ...
          <motion.div
            ...
            className="relative flex text-xl md:text-3xl lg:text-4xl font-bold"
          >
            {showInfo && (
              <button 
                onClick={() => setShowInfo(false)}
              ...
              >
                <span className="inline-block rotate-45 hover:text-white transform transition-colors duration-200">
                  +
                </span>
              </button>
            )}
      ...
      <div className="flex flex-col pt-0 md:pt-2 w-full">
        ...
        <div className="relative h-[1.8rem] md:h-[2.5rem] lg:h-[2.5rem] flex justify-center items-center">
          <motion.p
          ...
          >
            <span onClick={() => { setShowInfo(true); setSelectedSection('bio'); }} className="cursor-pointer hover:text-white transform transition-colors duration-200">bio</span> |  
            <span onClick={() => { setShowInfo(true); setSelectedSection('in detail'); }} className="cursor-pointer hover:text-white transform transition-colors duration-200"> in detail</span> | 
            <span onClick={() => { setShowInfo(true); setSelectedSection('works'); }} className="cursor-pointer hover:text-white transform transition-colors duration-200"> works</span> |
            <span onClick={() => { setShowInfo(true); setSelectedSection('social media'); }} className="cursor-pointer hover:text-white transform transition-colors duration-200"> socials</span> 
          </motion.p>
        </div>

        <div className="flex font-bold justify-center short:text-xl text-2xl md:text-3xl lg:text-4xl cursor-pointer"
        >
          <div onClick={() => router.push(`/artist/${data.id}/booking`)}
            className="font-bold group hover:bg-[#ed3833] hover:text-black transform transition-colors duration-200 px-1"
          >
...
export default ArtistProfile19;
